...
;; The following are our recommended default settings. Some of these settings
;; can be modified by passing an optional parameter to a function (e.g., the
;; maximum fps). Others can only be modified here. We do not recommend
;; changing any of these settings unless you absolutely need them for your game
;; to function. Change these at your own risk!

(define default-maximum-fps 60)
(define fps-refresh-time 1000)
(define ignore-held-key #t)
(define max-one-window? #t)
...
(define first-window? #t)

(define (make-window w h title (maximum-fps default-maximum-fps))
  (when max-one-window?
    (unless first-window?
      (error "A window has already been created. Only one window can be created using Graphics.rkt"))
    (set! first-window? #f))
  (let* ((show-fps #t)
         (fps 0)
         (fps-accum-dt 0)
